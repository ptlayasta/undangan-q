...
# Native Google OAuth (authorization code flow)
GOOGLE_CLIENT_ID = os.environ.get("GOOGLE_CLIENT_ID", "")
GOOGLE_CLIENT_SECRET = os.environ.get("GOOGLE_CLIENT_SECRET", "")

# S3-compatible object storage (works with Cloudflare R2, AWS S3, MinIO, etc.)
...
def init_storage(force: bool = False):
    """Create (or return cached) boto3 S3-compatible client. Kept as a function
    for startup-time connectivity checks, mirroring the old init_storage()."""
    global _s3_client
    if _s3_client is not None and not force:
        return _s3_client
    if not (S3_ENDPOINT_URL and S3_ACCESS_KEY_ID and S3_SECRET_ACCESS_KEY and S3_BUCKET_NAME):
        raise RuntimeError(
            "S3 storage not configured. Set S3_ENDPOINT_URL, S3_ACCESS_KEY_ID, "
...
async def google_auth(payload: dict, response: Response):
    """Exchange a Google OAuth authorization code for tokens, fetch the user's
    profile, and create a local session. Requires GOOGLE_CLIENT_ID and
    GOOGLE_CLIENT_SECRET to be set, and the redirect_uri sent here must match
...
async def on_startup():
    try:
        init_storage()
        logger.info("S3-compatible object storage initialized")
    except Exception as e:
        logger.error(f"Storage init failed (uploads will fail until configured): {e}")
